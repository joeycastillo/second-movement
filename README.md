...
To build the default firmware for the sensorwatch_pro with a custom screen:

```
nix build '.?submodules=1'
```
Other firmware can be built by specifying the `board_type` `display_type`
combo. For example:

```
nix build '.?submodules=1#sensorwatch_pro_classic' 
```
